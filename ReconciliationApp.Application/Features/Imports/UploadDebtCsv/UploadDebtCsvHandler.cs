...
public sealed class UploadDebtCsvHandler
{
    ...
    private readonly ILogger<UploadDebtCsvHandler> _logger;
    ...
    public UploadDebtCsvHandler(
        ...
        ILogger<UploadDebtCsvHandler> logger)
    {
    ...
    public async Task<ImportResult> Handle(Guid batchId, int runNumber, UploadCsvRequest req, CancellationToken ct)
    {
        _logger.LogInformation(
            "Debt CSV import started. BatchId={BatchId} RunNumber={RunNumber}",
            batchId, runNumber);
        ...
        _logger.LogInformation(
            "Debt CSV parsed. BatchId={BatchId} Rows={Rows}",
        ...
        for (var i = 0; i < jsonRows.Count; i++)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogWarning(ex,
                    "Debt CSV row error. BatchId={BatchId} RowNumber={RowNumber}",
                    batchId, rowNumber);
        ...
        _logger.LogInformation(
            "Debt CSV import completed. BatchId={BatchId} Inserted={Inserted} Updated={Updated} Closed={Closed}",
            batchId, inserted, updated, closed);
